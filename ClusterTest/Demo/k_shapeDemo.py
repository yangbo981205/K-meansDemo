...
class Plot_Cluster_Time_Series(object):
    ...
    def read_data(self):
        ...
        data_array = np.array(multi_ts[['qty']]).reshape(multi_ts['item_id'].nunique(),
                                                         multi_ts.shape[0] // multi_ts['item_id'].nunique())
...
def main():
    seed = 666
    data = pd.read_csv(r'F:\TimeSeriesGit\time_series_data_mining-master\sale_df.csv', parse_dates=['date'])
    print(data)
